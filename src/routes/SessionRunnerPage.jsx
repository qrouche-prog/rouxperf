    ...
    async function load() {
      const [{ data: programData, error }, { data: exercises }, { data: tp }] = await Promise.all([
        ...
        supabase
          .from('exercises')
          .select(
            'id, name, instructions, illustration_slug, category, muscle_group, equipment_required, is_ai_generated, musclewiki_videos, musclewiki_steps'
          ),
              ...
              {members.map(({ exercise: me, index: mi }) => {
                ...
                return (
                  <div key={mi} className="superset-member">
                    ...
                    <ExerciseInfoPanel
                      media={mediaForSlug(mDet?.illustration_slug)}
                      name={mDet?.name}
                      instructions={mDet?.instructions}
                      steps={mDet?.musclewiki_steps}
    ...
    <main className="session-run">
      ...
      <div className="card session-exo-card">
        ...
        <ExerciseInfoPanel
          ...
          steps={details?.musclewiki_steps}
          videos={details?.musclewiki_videos}
        />
